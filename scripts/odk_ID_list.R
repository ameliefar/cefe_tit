...
ind1 <- ind %>% 
  ...
  dplyr::mutate(espece = dplyr::case_when(speciesID == "CYACAE" ~ "bleue",
                ...
                nic_naiss = dplyr::case_when(LaidNestbox == "NA_NA" ~ NA_character_,
                                        TRUE ~ LaidNestbox),
                age_2026 = dplyr::case_when(tagStage == "chick" ~ 2026 - tagYear,
                                            tagStage == "subadult" ~ 2026 - (tagYear - 1),
                                            TRUE ~ 2026 - (tagYear - 2))) %>% 
  dplyr::select(individualID, espece, lieu_naiss = "LaidSite", nic_naiss, age_2026, calculatedSex) 
...
morpho_old <- openxlsx::read.xlsx(paste0(repo, "SIE MORPH 1976-2025.xlsx"), detectDates = TRUE)
#morpho_new <- openxlsx::read.xlsx(paste0(repo2, "MORPH_2025.xlsx"), detectDates = TRUE)
#morpho <- rbind(morpho_old, morpho_new)

pous <- openxlsx::read.xlsx(paste0(repo, "SIE POUS 1976-2025.xlsx"), detectDates = TRUE)
...
bird_id <- dplyr::left_join(ind1, first_tag, by = c("individualID" = "bague"))  %>% 
  dplyr::mutate(lieu_baguage = dplyr::case_when(is.na(lieu_naiss) ~ site,
                                         TRUE ~ lieu_naiss)) %>% 
  dplyr::select(bague = "individualID", espece, sexe = "calculatedSex", age_2026, lieu_bag = "lieu_baguage", nic_naiss)
...
bird_id_1 <- dplyr::left_join(bird_id, darvic, by = "bague") %>% 
  ...
  dplyr::mutate(autre_bague = dplyr::case_when(espece == "charbo" ~ darvic,
                                               TRUE ~ bague_coul)) %>% 
  dplyr::select(bague, autre_bague, espece, sexe, age_2026, lieu_bag, nic_naiss)
...
bird_id_2 <- dplyr::left_join(bird_id_1, blood, by = "bague") %>% 
  dplyr::mutate(sang_ok = dplyr::case_when(quantite_sang >= 10 ~ "oui",
                                           TRUE ~ "non")) %>% 
  dplyr::select(bague, autre_bague, espece, sexe, age_2026, lieu_bag, nic_naiss, sang_ok)
...
write.csv(bird_id_3, paste0(output, "bird_id.csv"), row.names = FALSE, na = "")